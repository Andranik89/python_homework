...
"""Լավ ա Անդրանիկ ջան։ Միայն մի քանի խնդիր կա կոդի ֆորմատավորման հետ։ Pycharm-ը աջ կողմում (scrollbar-ի վրա) գծիկներով
ցույց ա տալիս, թե որ տողերում ֆորմատավորման խնդիրներ կան։ Կարող ես դրանց վրա սեղմել ու ուղղել։"""
...
for i in fullname:
    i_value = ord(i)
    fullnamne_ascii_value += i_value  # Այստեղ որպեսզի ամեն անգամ i_value փոփոխական չհայտարարենք, կարող ենք ուղղակի գրել
    # fullnamne_ascii_value += ord(i)
...
if year % 4 == 0:  # Այստեղ փորձիր ուղղել կոդը։ if-երի հերթականության մեջ խնդիր կա։ 1800 նահանջ չէ, բայց ստանում եմ նահանջ
    if not bool(year % 100):  # Կարելի է պարզապես գրել if not year % 100: Եթե վերջինս 0 է, ապա False կդառնա
